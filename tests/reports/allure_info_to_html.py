...
test_data = []
keys_to_delete = ('status', 'statusDetails', 'message', 'trace', 'start', 'stop', 'uuid', 'historyId', 'testCaseId')
    ...
    with open(file, encoding="utf8") as f:
        ...
        if "steps" in temp.keys():
            for step in temp.get('steps'):
                step["start"] = datetime.fromtimestamp(step["start"] / 1000).strftime("%m/%d/%Y, %H:%M:%S")
                step["stop"] = datetime.fromtimestamp(step["stop"] / 1000).strftime("%m/%d/%Y, %H:%M:%S")
        test_data.append(temp)


# Reading html report
with open(report, encoding="utf8") as report_stream:
...
for test_report in test_data:
    ...
    html_to_insert = "<br><br><details><summary>Detailed report</summary><p>" + json2html.convert(json=test_report) + "</p></details>"

    report_data = report_data[:index + len(test_name)] + html_to_insert + report_data[index + len(test_name):]

# Writing updated html report to file
with open(report, 'w', encoding="utf8") as report_stream:
